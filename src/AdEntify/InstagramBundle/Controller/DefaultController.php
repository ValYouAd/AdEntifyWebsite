...
class DefaultController extends Controller
{
    ...
    public function indexAction()
    {
        if ($this->getRequest()->query->has('error')) {
            ...
            return $this->redirect($this->generateUrl('instagram_photos'));

            /*$userMediaUrl = 'https://api.instagram.com/v1/users/'.$json->user->id.'/media/recent?access_token='
                .$json->access_token.'&count=200';

            $curl = curl_init($userMediaUrl);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
            $json2 = json_decode(curl_exec($curl));

            return array(
                'image' => $json2->data[0]->images->standard_resolution->url
            );*/
        }
        else {
